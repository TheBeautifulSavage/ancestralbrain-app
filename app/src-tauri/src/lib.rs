...
/// Return detected hardware info (arch, RAM, recommended models).
#[tauri::command]
fn get_hardware_info(app: AppHandle) -> serde_json::Value {
...
fn search(app: AppHandle, query: String) -> Result<Vec<db::SearchResult>, String> {
    ...
    let embed_model = state.hardware.embed_model.clone();

    let mut results = db::filename_search(&conn, &query, 20).unwrap_or_default();

    if ollama::is_running() {
        if let Ok(emb) = ollama::embed(&query, &embed_model) {
...
fn chat(app: AppHandle, question: String) -> Result<serde_json::Value, String> {
    ...
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    let embed_model = state.hardware.embed_model.clone();
    let chat_model = state.hardware.chat_model.clone();

    if !ollama::is_running() {
        return Err("Ollama is not running. Start it with: ollama serve".into());
    }

    let emb = ollama::embed(&question, &embed_model).map_err(|e| e.to_string())?;
    ...
    let answer = ollama::chat(&question, &context, &chat_model).map_err(|e| e.to_string())?;
...
fn start_index(app: AppHandle, folder: PathBuf) {
    ...
    // Clone model names out of state before spawning thread
    let (embed_model, chat_model) = {
        let state = app.state::<AppState>();
        (state.hardware.embed_model.clone(), state.hardware.chat_model.clone())
    };
    ...
    std::thread::spawn(move || {
        ...
        let app3 = app2.clone();
        let result = indexer::index_folder(
            &conn,
            &folder,
            &vault_root,
            &embed_model,
